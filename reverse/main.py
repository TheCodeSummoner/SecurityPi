...
def main(server, data):
    ...
    if switch == 1:
        ...
        print("Reverse: Encrypting following sentence: " + output)

        # Add the sentence to cache, to check the answer later
        server.cache["reverse_message"] = output
    ...
    # If the help argument was included, display the help message
        ...
        message = "!reverse - starts the programming challenge described under id 01 on the website" + "\r\n" + \
    ...
    # If a no_key argument was included, generate a key, encrypt a message and start the challenge
        ...
        server.cache["reverse_key"] = key

        # Inform what key was generated
        print("Reverse: Generated following key: " + key)
    ...
    # If a non-help, non-no_key arguments were included, check if the answer is correct
        ...
        if data[1] == "no_key":
            ...
            if "reverse_key" in server.cache.keys():

                # Inform what answer was received
                print("Reverse: Received following answer: " + data[2])

                # Check if the answer is correct
                if data[2] == server.cache.get("reverse_key"):
                    ...
                    server.cache.pop("reverse_key")

            else:
                # Inform the user that !reverse no_key hasn't been executed yet
                message = "No key was generated yet! Type !reverse no_key before sending an answer to it." + "\r\n"

        # If an answer to standard challenge was provided
        else:
            # Check if the message to encrypt was actually generated before
            if "reverse_message" in server.cache.keys():
                ...
                # Inform what answer was received
                print("Reverse: Received following answer: " + answer)

                # Check if the answer is correct
                if answer == server.cache.get("reverse_message"):
                    # Send the "correct!" message if the answer matches the message
                    message = "Correct! Well done!" + "\r\n"
                else:
                    # Send the "incorrect!" message if the answer doesn't match the message
                ...
                # Inform the user that !reverse hasn't been executed yet
                message = "No message was generated yet! Type !reverse before sending an answer to it." + "\r\n"
